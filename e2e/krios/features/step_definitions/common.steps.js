...
const censor = require('../../helpers/helpers.js').censor;
const errors = require('../../helpers/errors.js');
const FILE_PATH = __dirname.split('Inksoft')[0];
const pageObj = browser.params.pageObjects;

chai.use(chaiAsPromised);
const expect = chai.expect;
const EC = protractor.ExpectedConditions;
const defaultCustomTimeout = 10000;
const customTimeout = browser.params.customTimeout;
const pageObjects = browser.params.pageObjects;
const timeToWaitMax = 300100; // Maximum time to wait for in 'I wait for (\d+) ms' step

// Browser Stack Local File Upload
const remote = require('selenium-webdriver/remote');

//Declaring variables globally for changing the resolution of the application
let width;
let height;
...
function goToEnv(page, elem) {
  const url = pageObj.enviro + pageObjects[page][elem];

  return browser.get(url);
}

function goToTestEnv(page, elem) {
  const url = pageObj.testEnv + pageObjects[page][elem];

  return browser.get(url);
}

function goToEssDev(page, elem) {
  const url = pageObj.essentialsDevEnv + pageObjects[page][elem];
...
/**
 * Waits for 300 ms and then clicks on element provided in page object
 * @param {string} page
 * @param {string} elem
...
function moveWithOffsetTo(page, elem, offsetX, offsetY) {
  ...
  return browser.actions().mouseMove(elmnt).mouseMove({
    x: integerX,
    y: integerY
  }).perform();
}
...
function verifyTextValueContains(page, elem, textPart, callback) {
  const elmnt = composeLocator(page, elem);

  elmnt.getAttribute('value').then(function (text) {
    if (text.indexOf(textPart) === -1) {
      throw new Error(`"${text}" ${errors.CONTAIN} "${textPart}"`);
    } else {
      callback();
...
defineSupportCode(function ({
  ...
  When(/^I wait for (\d+) ms$/, {
    timeout: timeToWaitMax
  }, function (timeToWait, callback) {
  ...
  When(/^I type "([a-zA-Z0-9_]+)"."([a-zA-Z0-9_]+)" in "([a-zA-Z0-9_]+)"."([a-zA-Z0-9_]+)"$/, function (page1, element1, page2, element2, callback) {
    typePageObjectIn(page1, element1, page2, element2).then(function () {
      callback();
    });
  });

  When(/^I type ([a-zA-Z0-9_]+) from ([a-zA-Z0-9_]+) page in ([a-zA-Z0-9_]+) from ([a-zA-Z0-9_]+) page$/, function (element1, page1, element2, page2, callback) {
    typePageObjectIn(page1, element1, page2, element2).then(function () {
      callback();
    });
  });
  ...
  When(/^I move to "([a-zA-Z0-9_]+)"."([a-zA-Z0-9_]+)" with an offset of x: (\d+)px, y: (\d+)px$/, function (page, elem, offsetX, offsetY, callback) {
    moveWithOffsetTo(page, elem, offsetX, offsetY).then(function () {
      callback();
    });
  });

  When(/^I move to ([a-zA-Z0-9_]+) from ([a-zA-Z0-9_]+) page with an offset of x: (\d+)px, y: (\d+)px$/, function (elem, page, offsetX, offsetY, callback) {
    moveWithOffsetTo(page, elem, offsetX, offsetY).then(function () {
      callback();
    });
  });
  ...
  When(/^I authenticate in browser alert with login "([^"]*)" and password "([^"]*)"$/, function (login, password, callback) {
    // Waits for an alert to appear
    browser.wait(EC.alertIsPresent(), customTimeout, errors.ALERT_PRESENT);
  ...
  Then(/^"([a-zA-Z0-9_]+)"."([a-zA-Z0-9_]+)" text should be "([a-zA-Z0-9_]+)"."([a-zA-Z0-9_]+)"$/, function (page1, element1, page2, element2, callback) {
  ...
  Then(/^"([a-zA-Z0-9_]+)"."([a-zA-Z0-9_]+)" text should contain "([^"]*)"$/, function (page, elem, textPart, callback) {
    verifyTextContains(page, elem, textPart, callback);
  });

  Then(/^"([a-zA-Z0-9_]+)"."([a-zA-Z0-9_]+)" text value should contain "([^"]*)"$/, function (page, elem, textPart, callback) {
    verifyTextValueContains(page, elem, textPart, callback);
  });

  Then(/^([a-zA-Z0-9_]+) text from ([a-zA-Z0-9_]+) page should contain "([^"]*)"$/, function (elem, page, textPart, callback) {
    verifyTextContains(page, elem, textPart, callback);
  });

  Then(/^"([a-zA-Z0-9_]+)"."([a-zA-Z0-9_]+)" text should contain "([a-zA-Z0-9_]+)"."([a-zA-Z0-9_]+)"$/, function (page1, element1, page2, element2, callback) {
    const elmnt = composeLocator(page1, element1);
